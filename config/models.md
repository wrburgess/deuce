...
- **No stage is routed to a different model or effort than the session's.** Differentiated routing is
  ...
  actionable, the per-stage table replaces this declaration — dated and sourced like everything here.

## Prior art

- [ace #77](https://github.com/wrburgess/ace/issues/77) — the predecessor's three-tier routing design
  (economy / standard / frontier), declared and never built. Cited as the shape a future per-stage
  table might take; its multi-tool machinery does not transfer, because deuce has one AC.
