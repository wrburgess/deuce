...
- **Nothing is delegated today.** No Skill exists yet, and every stage runs in the AC's own loop.
- **A contract shape enters this file when the first Skill actually delegates** — what is offloaded,
  the model and effort it runs at, and the shape the result must come back in. Each entry lands
  dated and sourced to the work that needed it.
