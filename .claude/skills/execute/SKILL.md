...
1. **Read the pass's configuration** — the gate settings ([`config/gates.md`](../../../config/gates.md));
   ...
   ([`config/models.md`](../../../config/models.md)) — **the row for each stage the pass dispatches,
   falling back to the declared default where a stage has no row**
   ([Chapter 6 → Routing, consumed](https://github.com/wrburgess/deuce/blob/main/sds/06-factory-automation.md#routing-consumed));
   the budget, when one is declared ([`config/capacity.md`](../../../config/capacity.md)).
